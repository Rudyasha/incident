...
- Повторный запрос с `exchange_id=ex-20260824-0010482` был повторной отправкой того же обмена после timeout, а не новой операцией. На это указывают одинаковые `exchange_id`, `client_id`, `clean_mats` и `dirty_mats` в обеих попытках.

- Время в логах Mobile, Lincore API, Kafka и 1С синхронизировано достаточно точно, поэтому события из разных компонентов можно сопоставить в одну последовательность.

- Предполагаю, что финансовый реестр сформировал итоговый статус по результату повторной обработки операции. Первый Kafka event успешно создал счёт `inv-2026-00088234`, а второй event с тем же `exchange_id` был распознан как `duplicate_document` и отправлен в DLQ. Из-за этого реестр мог использовать результат второй, ошибочной попытки и показать `invoice=N/A`, не учитывая уже успешно созданный счёт.

- Предполагаю, что счёт `inv-2026-00088234`, который 1С вернула как успешно созданный при обработке первого события, действительно сохранился в 1С. В предоставленных данных нет отдельной проверки его текущего состояния.
...
**Интеграция.** Первое сообщение создало счёт, второе получило ошибку, потому что такой документ уже существовал. Нужно проверить, относится ли найденный счёт к тому же обмену и совпадают ли данные.

**Данные и отчётность.** Предполагаю, что финансовый реестр определяет итоговый статус по наличию последнего ошибочного события или записи в DLQ, не учитывая ранее успешно созданный invoice для того же `exchange_id`. Это требует проверки, поскольку логика формирования реестра в артефактах не представлена.
...
1. **Как должна обеспечиваться идемпотентность операции «Завершить обмен»?** Что должен делать сервер при повторном POST с тем же `exchange_id` после timeout, double tap или перезапуска приложения, в том числе если payload повторного запроса отличается?

2. **В какой момент обмен считается действительно завершённым и какая система является source of truth — источником достоверного состояния?** Сохранение в Lincore, успешная публикация Kafka или создание счёта в 1С? Как должен определяться итоговый статус при расхождении состояний между Lincore, 1С, DLQ и финансовым реестром?

3. **Как определяется и создаётся финансовый документ?** Где рассчитывается сумма счёта, что является уникальным ключом invoice и как должен обрабатываться повторный запрос в 1С, если документ с таким `exchange_id` уже существует?

4. **Допустимо ли завершить обмен, если `clean_mats = 0` и `dirty_mats = 0`?** Есть ли бизнес-зависимость между `clean_mats` и `dirty_mats`: должны ли количества быть равны, либо допустимы сценарии `8/5`, `0/8`, `8/0`?

---

**Дополнительно.** При разборе спецификации и флоу вопросов получилось значительно больше. Здесь я выделила четыре ключевых, остальные могу озвучить на интервью.
