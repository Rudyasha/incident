...
**Дополнительно.** При разборе спецификации и флоу вопросов получилось значительно больше. Здесь я выделила три ключевых, без ответа на которые нельзя однозначно определить ожидаемое поведение и считать функцию протестированной. Отдельного уточнения требуют, например, уникальность `exchange_id` при нескольких посещениях клиента за сутки, правила timeout и retry, восстановление состояния приложения и порядок ручной обработки DLQ.
